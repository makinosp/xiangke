...
Manage transitions between the four game states in the minimal state machine
loop.
...
## Logic Model: Two-Phase Character Selection

### Purpose

Manage the two-phase character selection flow: corps selection (6 characters)
followed by battle selection (3 characters), with opponent reveal between
phases.

### Input

- `phase`: CharacterSelectPhase enum
...
- During Phase 1, the player can deselect and reselect characters freely until
  confirmation.
- During Phase 2, the player can deselect and reselect from their 6, but cannot
  return to Phase 1 without restarting the selection.
- The opponent's corps is generated by the system (AI selection logic in Unit
  4).
- Stats preview and character description MUST be visible for all 6 characters
  during both phases.
- If the available character count is less than 6, Phase 1 cannot complete
  (error condition).
