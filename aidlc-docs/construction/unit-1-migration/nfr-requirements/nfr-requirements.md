...
**Verification Method:** Benchmark tests using `#[bench]` or manual timing
harness in CI. Key hot paths (damage calc, turn queue) should have dedicated
benchmarks.
...
**Verification Method:** CI pipeline compiles for both native and WASM targets.
Phase 5 adds Godot export test.
...
**Verification Method:** `cargo test` covers unit and integration tests. CI
...
**Verification Method:** `cargo build --release` checked in CI. Binary size and
test time monitored via CI artifacts.
