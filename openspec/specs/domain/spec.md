...
#### Scenario: Reverse-generating cycle (子盗母気)

- **WHEN** attacker's type is generated by defender's type (e.g., Fire → Wood,
  Earth → Fire, Metal → Earth, Water → Metal, Wood → Water)
- **THEN** the effectiveness multiplier is 2.0×

#### Scenario: Single-type effectiveness

- **WHEN** the defender has no secondary type
- **THEN** the final multiplier equals the single-type effectiveness lookup
- **AND** no secondary multiplication is applied
